...
function Write-Log {
			...
			try {			
				foreach($m in $Messages) {			
					...
					if ($EventLogName) {
						...
						switch ($Level) {
							"Error" { $log.WriteReport($Message, 'Error', $EventID) }
							"Warn"  { $log.WriteReport($Message, 'Warning', $EventID) }
							"Info"  { $log.WriteReport($Message, 'Information', $EventID) }
						}
					}
				}
			} 
			catch {
				throw "Failed to create log Report in: '$Path'. The error was: '$_'."
			}
		}
	}
	...
	<#
		...
		.PARAMETER EventSource
			The name to appear as the source attribute for the system event log Report. This is ignored unless 'EventLogName' is specified.
		
		.PARAMETER EventID
			The ID to appear as the event ID attribute for the system event log Report. This is ignored unless 'EventLogName' is specified.
...
function Get-IdLibProperties {
	param([string] $libFullPath)

	$result = ""
	$Tokens = @{
		version = "1.0.0"
	}
	$versionInfo = (Get-ChildItem $libFullPath | Select-Object -First 1 -ExpandProperty VersionInfo)
	
	If ($versionInfo) {
		$Tokens.title = $versionInfo.Title
		$Tokens.authors = $versionInfo.CompanyName
...
function Get-IdLibPath {
	$libFullPath = ""
	$nuspecFileName = ".\Package.nuspec"
	$xml = [xml](Get-Content $nuspecFileName)
	$projectName = $xml.package.metadata.tags
	$libName = $xml.package.metadata.id
	$libFullName = $libName + ".dll"
	if ($projectName.Length -eq 0) 
	{
		$projectName = $libName
	}
	$libFolder = "..\" + $projectName + "\bin\Release"

	$fileInfo = Get-ChildItem -Recurse -Force $libFolder | Where-Object { $_.Name -eq $libFullName } | Select-Object Name, Directory
	if ($fileInfo)
	{
		$libFullPath = [string]$fileInfo.Directory + "\" + [string]$fileInfo.Name
	}
	$libFullPath
}

function Get-IdLibVersion {
	param([string] $libFullPath)
	...
	If ($versionInfo) 
	{ 
		$result = $versionInfo.ProductVersion.Substring(0, $versionInfo.ProductVersion.LastIndexOf("."))
	}
	$result
}
...
Write-Log ("Trying to get the version number of the assembly specified in 'id' parameter in the 'Package.nuspec' file")
$libFullPath = Get-IdLibPath
$version = Get-IdLibVersion($libFullPath)

if ($version.Length -gt 0) 
{ 
	$logStr = "The version number {0} was successfully obtained from the assembly {1}." -f $version, $libFullPath
	Write-Log ($logStr)
}
else 
{
	Write-Log ("Failed to obtain the version number. Nuget version will be taken from Package.nuspec file.")
}
...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	...
	# we will take into account version number if we have successfully obtained it earlier 
	if ($version.Length -gt 0)
	{
	...
	# there is an info that the operation can cause stack overflow in case of huge file list
	# in that case it's better to move 'WaitForExit' command beyond following 2 lines (where $output and $error are obtained)
