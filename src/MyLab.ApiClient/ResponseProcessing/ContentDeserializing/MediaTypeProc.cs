...
class MediaTypeProc
{
    ...
    IDictionary<string, Func<HttpContent, Task<object?>>> _creators = new Dictionary<string, Func<HttpContent, Task<object?>>>();
    Func<HttpContent, Task<object>>? _defaultCreator;
    ...
    public async Task<object?> GetResult()
    {
        ...
        if (mediaTypeIsSpecified && _creators.TryGetValue(_content.Headers.ContentType!.MediaType!, out var creator))
        {
        ...
        throw new UnexpectedResponseContentTypeException(_content.Headers.ContentType?.MediaType ?? "[undefined]");
    }
}