...
* Поддержка атрибутов `EumMember` для значений перечислений при сериализации

## [3.21.34] - 2026-03-26

### Изменено

* Сериализация даты+времени - по ISO8601

## [3.21.33]-2026-01-21

### Добавлено

* Подавление исключения в случае, если неожиданный mime-type и код не 2хх 
...
* Возврат ожидаемого результата в случае ожидаемого не 200 кода ответа
* Алгоритм определения неожиданного кода ответа, в случае получения ответа 200, будет выбрасывать исключение, если есть ожидаемые ответы >200 и среди них нет 200

## [3.21.31]-2025-02-25

### Добавлено

* возможность указывать `Mime-Type` для бинарного содержимого в атрибуте `BinContent`

### Изменено

* бинарный выходной параметр поддерживает любое содержимое
...
* в качестве перехода на новую строку в дампере теперь используется `\r\n` 

## [3.19.28] - 2023-09-27

### Изменено

* опубликован дампер `http` запросов и ответов `HttpMessageDumper`

## [3.18.27] - 2023-07-23

### Добавлено

* возможность указать имя клиента при регистрации контракта

## [3.17.27] - 2023-04-03

### Добавлено

* поддержка опциональных клиентов (в зависимости от наличия конфигурации)
* поддержка использования имени контракта в качестве ключа конфигурации API, если не указан ключ в атрибуте `ApiAttribute`  

## [3.16.27] - 2023-01-19

### Добавлено

* дополнительная информация о запросе при ошибке отправки запроса

## [3.16.26] - 2022-12-01

### Изменено

* сериализация `Guid` параметров в формате `N` - без доп. символов
